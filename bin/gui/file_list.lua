...
function FilterFileList(dir)
	...
	if f:len() > 0 then
		...
		for i, ent in ipairs(dir) do
			...
			local name = ent.name
			by_name[name] = ent
			if name:find(f) and show_extensions[name:sub(-4)] then
				list[n] = name
				n = n + 1
			end
		end
		...
		for i, ent in ipairs(dir) do
			...
			local name = ent.name
			by_name[name] = ent
			if show_extensions[name:sub(-4)] then
				list[n] = name
				n = n + 1
			end
		end
...
function list:action(str, pos, state)
	if state == 1 and str:find("%.mod$") then
		local sel = by_name[str]
		if selection ~= sel then
			selection = sel
			local s, data = pcall(eqg.OpenEntry, sel)
			if s then
				s, data = pcall(mod.Read, sel)
				if s then
					ClearDisplay()
					UpdateDisplay(data, str)
					return
				end
			end
			error_popup(data)
...
function Export()
	...
	if dlg.status == "0" then
		...
		if path and val then
			...
			local outpath = path .."\\".. str:sub(1, -4) .. "ply"
			local data = by_name[str]
